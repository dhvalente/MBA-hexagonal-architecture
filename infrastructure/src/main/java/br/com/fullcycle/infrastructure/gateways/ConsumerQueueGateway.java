...
public class ConsumerQueueGateway implements QueueGateway {
    ...
    private <T> T safeRead(final String content, final Class<T> tClass) {
        try {
            final var node = this.mapper.readTree(content);
            // Algumas colunas JSON (ex.: H2 em modo MySQL) devolvem o conteúdo com um nível
            // extra de escaping, isto é, uma string JSON contendo o texto do evento.
            final var json = node.isTextual() ? node.asText() : content;
